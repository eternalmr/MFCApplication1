...
BOOL CTreeCtrlDlg::OnInitDialog()
{
	...
	list.Create(64, 64, ILC_COLOR32, 1, 1);
	list.Add(icon);
	m_tree.SetImageList(&list, TVSIL_NORMAL);

	// 设置节点
	HTREEITEM root =  m_tree.InsertItem(TEXT("根节点"), 0, 0, NULL);
	HTREEITEM parent =  m_tree.InsertItem(TEXT("父节点"), 0, 0, root);
	HTREEITEM sub1 = m_tree.InsertItem(TEXT("子节点1"), 0, 0, parent);
	HTREEITEM sub2 = m_tree.InsertItem(TEXT("子节点2"), 0, 0, parent);
